...
internal static class Program
{
    ...
    private static void RunVehicleMenu(BrandCatalogService brands, VehicleService vehicles)
    {
        while (true)
        {
            ...
            Console.WriteLine("7) Reparatur eintragen");
            Console.WriteLine("8) Reparaturbuch anzeigen");
            Console.WriteLine("9) Reparatur löschen");
            Console.WriteLine("0) Zurück");
            PrintSeparator();

            var choice = ConsoleInput.ReadInt("Auswahl", 0, 9);
            if (choice == 0) return;
            ...
            switch (choice)
            {
                ...
                case 6:
                    ...
                    break;
                case 7: AddRepairWorkflow(vehicles); break;
                case 8: ShowRepairBookWorkflow(vehicles); break;
                case 9: DeleteRepairWorkflow(vehicles); break;
            }
        }
    }
    ...
    private static void AddRepairWorkflow(VehicleService vehicles)
    {
        ...
        Console.WriteLine("2) Verschleißteil-Austausch");
        PrintSeparator();

        var typeChoice = ConsoleInput.ReadIntOrBack("Auswahl", 1, 2);
    ...
    {
        Console.Clear();
        PrintHeader("Reparaturbuch anzeigen");
        ...
        Console.Clear();
        PrintHeader($"Reparaturbuch: {vehicle.LicensePlate} ({vehicle.Brand} {vehicle.Model})");

        if (vehicle.Repairs.Count == 0)
        {
            PrintInfo("Keine Reparaturen vorhanden.");
            return;
        }
        ...
        Console.WriteLine();
        Console.ForegroundColor = ConsoleColor.Yellow;
        Console.WriteLine($"Gesamtkosten Reparaturen: {vehicle.GetTotalRepairCost():C}");
        Console.ResetColor();
    ...
    private static void DeleteRepairWorkflow(VehicleService vehicles)
    {
        while (true)
        {
            ...
            if (vehicle.Repairs.Count == 0)
            {
                PrintInfo("Dieses Fahrzeug hat keine Reparaturen.");
                return;
            }
            ...
            if (match == null)
            {
                PrintInfo("Keine Reparatur mit dieser Id gefunden.");
                continue;
            }

            try
            {
                vehicles.RemoveRepair(vehicle.Id, match.Id);
                PrintSuccess("Reparatur gelöscht.");
            }
            catch (DomainValidationException ex)
            {
                PrintError(ex.Message);
            ...
            if (!ReadYesNo("Noch eine Reparatur löschen?", defaultYes: false))
                return;
        }
    }
